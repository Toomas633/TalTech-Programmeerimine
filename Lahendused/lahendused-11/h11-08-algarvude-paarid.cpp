 ...
 * Massiivid
 * Harjutus 11-08: Algarvude paarid
 * Ants-Oskar Mäesalu
 * 2022-02-21
 */
...
int main() {
	// Küsime kasutajalt arvu, mida algarvulisteks liidetavateks jagada
	unsigned int summa = loe_mittenegatiivne_taisarv("Sisesta algarvulisteks liidetavateks jagatav arv");
	cout << endl;

	// Leiame kõik algarvud kuni nimetatud summani
	unsigned int* algarvud = nullptr;
	unsigned int kogus = 0;
	leia_algarvud(summa, algarvud, kogus);

	// Leiame kõik algarvude paarid, mille summa on sisestatud arv
	// Kuna algarvud on massiivi salvestatud, saame iga massiivi elemendi kohta
	// vaadelda iga sellele järgnevat algarvu ning võrrelda iga selliste
	// algarvude paari summat sisestatud arvuga
	for (unsigned int i = 0; i < kogus; i++) {
		for (unsigned int j = i; j < kogus; j++) {
			if (algarvud[i] + algarvud[j] == summa) {
				cout << summa << " = " << algarvud[i] << " + " << algarvud[j] << endl;
			}
		}
	}
